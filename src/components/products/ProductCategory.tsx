...
import { productCategories, productCategoryContent } from "@/data/products";
...
function SingleCategoryView({ category }: SingleCategoryViewProps) {
  return (
    <div>
      ...
      {category.subcategories.length > 0 ? (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {category.subcategories.map((subcategory, index) => (
            <FadeIn key={subcategory.id} delay={index * 0.06}>
              <SubcategoryCard
        ...
        <FadeIn>
          <div className="border border-dashed border-zinc-300 bg-white px-6 py-16 text-center">
            ...
            <p className="mt-5 text-sm leading-7 text-zinc-500">
              No product series are currently available under this category.
            </p>
          </div>
        </FadeIn>
      )}
...
export default function ProductCategory({ selectedCategorySlug }: ProductCategoryProps) {
  const selectedCategory = productCategories.find(
    (category) => category.slug === selectedCategorySlug
  );

  const showAllCategories = selectedCategorySlug === "all" || !selectedCategory;

  return (
    <div className="relative min-w-0 transition-all duration-300 ease-in-out">
      {/* Fixed minimum height to reduce layout jumps */}
